 ...
 if Login_Valido == True and ID_Usuario[1] == 0:
    ...
    while Escolha_Valida01 == False:
     ...
     while Escolha_Valida01 == False:
        ...
        print ('[4] - Configuração de Conta de Cliente.')
        print ('[5] - Configuração de Conta de Administração.')
        print ('[6] - LogOff')
        print ('[0] - Recarregar.')
        entrada02 = input('>Escolha uma operação valida: ')
        if entrada02[0] == '1':
            ...
            print (' ')
            db.commit
            ponteiro.execute ("SELECT * FROM tbl_transferencia ORDER BY Transferencia_Data LIMIT 20;")
            Historico = ponteiro.fetchall()
            for entr in Historico:
                ...
                print ("#ID da transferencia: "+str(entr[0])+") Cliente "+str(emissor[0])+" ID: "+str(entr[1])+". Transferiu -> R$"+str(entr[2])+" para o cliente"+str(receptor)+", cliente de ID "+str(entr[3])+" em ["+str(entr[4])+"].")
            print ('\t')
            print = input('Deseja buscar os dados especificos de um usuario?')
            expandir = str(input("Insira seu ID ou CPF[Digite 'cancelar' para retornar a pagina inicial]: "))
            if len(expandir) == 14:
            ...
            historico = ponteiro.fetchall()
            print ("O cliente "+str(transfe[1])+" ID:"+str(expandir)+" possui o seguinte historico de transferencias:")
            for transfe in historico:
                if str(transfe[1]) == str(ID_Usuario[0]):
                    db.commit()
                    ponteiro.execute("SELECT Cliente_Nome FROM tbl_cliente WHERE Cliente_ID = "+str(transfe[3])+";")
