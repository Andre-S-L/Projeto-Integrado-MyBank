            ...
            while True:
                ...
                if entrada03[0] == '1':
                    ...
                    if Confirmacao_Alteracao1 == str(Login_Usuario) and "[('"+Confirmacao_Alteracao2+"',)]" == str(Usuario_Validacao):
                        Novo_Nome = input ('Identidade validada! Insira um nome novo: ')
                        if Novo_Nome.lower() == 'cancel':
                            print ('Cancelando alteração...')
                        elif Novo_Nome.isdigit() or len(Novo_Nome) < 2:
                            print ('\t')
                            print('[x] - Operação cancelada. Nome invalido.')
                            print ('\t')
                        elif Novo_Nome == Nome_Mostrado[0]:
                            print ('\t')
                            print ('[x] - Operação cancelada. O novo nome não pode ser o mesmo que o anterior.')
                            print ('\t')
                        else:
                            ponteiro.execute("UPDATE tbl_cliente SET Cliente_Nome = '"+Novo_Nome+"' WHERE Cliente_ID = "+str(ID_Usuario[0])+";")
                            db.commit()
                            print ('Alteração feita com sucesso! Ficamos feliz em te ter como cliente', Novo_Nome+'!')
                            print ('####--------------------------------------------------------------------####')
                            print ('\n')
                            break
                    else:
