    ...
    while True:
        ...
        if not Usuario_Validacao:
            ...
            if str(Usuario_Validacao) == "[('"+Usuario_Senha+"',)]":
                ...
                Nome_Mostrado = ponteiro.fetchall ()
                print ('\n')
                print ('Bem-Vindo de volta, '+str(Nome_Mostrado)+' ,a sua conta MyBank!') #Perguntar pro alan como tirar as aspas, tanto pra ficar bonito, e pra saber se tem como simplificar o (if str(Usuario_Validacao) == "[('"+Usuario_Senha+"',)]" )
                Login_Valido = True
                break
            else:
...
if Login_Valido == True:
    ...
    VALOR_Conta_Usuario = float(str(ponteiro.fetchone ()))
