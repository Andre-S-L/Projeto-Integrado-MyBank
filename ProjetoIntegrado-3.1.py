    ...
    if CPF_Unico == True and Nascimento_Valido == True:
        ...
        while Loop_Cad_2 == True:
            if PIN_Valido == True:
                ...
                print ('####--------------------------------------------------------------------####')
                print ('\n')
                ponteiro.execute("INSERT INTO tbl_cliente (Cliente_Nome, Cliente_CPF, Cliente_Nasc) VALUES ('"+str(Nome_Escolhido)+"', '"+str(CPF_Inserido)+"', '"+str(Nascimento_Formatado)+"');")
                db.commit()
                ponteiro.execute("SELECT Cliente_ID FROM tbl_cliente WHERE Cliente_CPF = '"+str(CPF_Inserido)+"';")   
                ID_Cliente_Conta = ponteiro.fetchone ()
                ponteiro.execute("INSERT INTO tbl_conta (Conta_Senha, Conta_Pin, Conta_QntAtual, fk_Conta_Cliente_ID) VALUES ('"+str(Senha_Escolhida)+"', '"+str(PIN_Escolhido)+"', '0.00', '"+str(ID_Cliente_Conta[0])+"');")
                db.commit ()
                break
            else:
                pass
...
if Login_Valido == True:
    ...
    while Escolha_Valida01 == False:
        ...
        if not Emprestimo_Calculo:
            ...
            print ('TESTE, veio para ca')
            Tempo_Ativo = str(Emprestimo_Calculo[4] - datetime.date.today())
            J = (Emprestimo_Calculo[1] * Emprestimo_Calculo[2] * Tempo_Ativo//2)
            Emprestimo_Valor_Atualizado = (Emprestimo_Calculo[1] + J)
            ponteiro.execute("UPDATE tbl_emprestimo SET Emprestimo_ValorAPagar = '"+str(Emprestimo_Valor_Atualizado)+"' WHERE fk_Emprestimo_Conta_ID = "+str(ID_Usuario[0])+";")
            db.commit()
            ...
            if Destin_Localizado == True:
             while Saldo_Suficiente == False:
                ...
                if Transferencia_Valor[0].lower () == 'c':
                    ...
                    Escolha_Valida01 = False
                    break
                elif not str.isdigit(Transferencia_Valor) or float(Transferencia_Valor) > float(VALOR_Conta_Usuario):
                    print ('[x] - Transferencia Cancelada. Saldo Insuficiente ou valor incorreto inserido.')
                    Escolha_Valida01 = False
                    break
            ...
            if str(Senha_Correto) == "('"+Senha_Validacao+"',)":
                ...
                while Emprestimo_Valido == False:
                    ...
                    if Emprestimo_Quantia[0].lower () == 'c':
                        ...
                        Emprestimo_Valido = False
                        break
                    elif not str.isdigit(Emprestimo_Quantia) and float(Emprestimo_Quantia) <= 0:
                      ...
                      while True:
                            if float(str(Emprestimo_Existente[7])) <= float(str(Quantia_Atual[0])):
                                ...
                                if Confirmacao_Devolucao2[0].lower() == 'c':
                                    ...
                                    ponteiro.execute("UPDATE tbl_emprestimo SET Emprestimo_Ativo = '0', Emprestimo_Data_Devol = '"+str(datetime.date.today())+"', Emprestimo_Tempo_Corrido_Dias = '"+str(Tempo_Corrido[0])+"';")
                                    db.commit
