    ...
    if len(CPF_Inserido) == 14:
     ...
     if not CPF_Duplicado_Resultados:
      ...
      while True:
       ...
       if len(Nascimento_Inserido) == 10 and int(Nascimento_Inserido[0]+Nascimento_Inserido[1]) <= 31 and int(Nascimento_Inserido[3]+Nascimento_Inserido[4]) <= 12:
          ...
          if len(Nascimento_Inserido) == 10 and (int(datetime.date.today().year) - int(str(Nascimento_Inserido[6])+str(Nascimento_Inserido[7])+str(Nascimento_Inserido[8])+str(Nascimento_Inserido[9]))) > 13:
            ...
            print ('\n')
            print('Cadastro cancelado! Para fazer um registro no MyBank é preciso ter, legalmente, mais de 13 anos de idade.')
            break
       else:
            print ('Data invalida, por favor confira se digitou corretamente. Lembresse de usar DD/MM/AAAA.')
     else:
        print ('\n')
