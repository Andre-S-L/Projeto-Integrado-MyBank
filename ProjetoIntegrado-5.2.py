  ...
  if len(entrada01) == 0:
    ...
    while Loop_Cad_1 == True:
      ...
      if CPF_Inserido[0].lower () == 'c':
         ...
         break
      elif len(CPF_Inserido) == 14 and CPF_Inserido[3] == '.' and CPF_Inserido[7] == '.' and CPF_Inserido[11] == '-':
       CPF_Formatado_1 = (int(CPF_Inserido[0])*10 + int(CPF_Inserido[1])*9 + int(CPF_Inserido[2])*8 + int(CPF_Inserido[4])*7 + int(CPF_Inserido[5])*6 + int(CPF_Inserido[6])*5 + int(CPF_Inserido[8])*4 + int(CPF_Inserido[9])*3 + int(CPF_Inserido[10])*2)
       Validacao_1 = (CPF_Formatado_1 * 10)%11
       CPF_Formatado_2 = (int(CPF_Inserido[0])*11 + int(CPF_Inserido[1])*10 + int(CPF_Inserido[2])*9 + int(CPF_Inserido[4])*8 + int(CPF_Inserido[5])*7 + int(CPF_Inserido[6])*6 + int(CPF_Inserido[8])*5 + int(CPF_Inserido[9])*4 + int(CPF_Inserido[10])*3 + int(CPF_Inserido[12])*2)
       Validacao_2 = (CPF_Formatado_2 * 10)%11
       ponteiro.execute("SELECT * FROM tbl_cliente WHERE Cliente_CPF = '"+str(CPF_Inserido)+"';")
       CPF_Duplicado_Resultados = ponteiro.fetchall ()
       if not CPF_Duplicado_Resultados and Validacao_1 == int(CPF_Inserido[12]) and Validacao_2 == int(CPF_Inserido[13]):
        CPF_Unico = True
        while True:
         ...
         if Nascimento_Inserido[0].lower () == 'c':
           ...
           if len(Nascimento_Inserido) == 10 and (int(datetime.date.today().year) - int(str(Nascimento_Inserido[6])+str(Nascimento_Inserido[7])+str(Nascimento_Inserido[8])+str(Nascimento_Inserido[9]))) > 13:
             ...
             print (' ')
         break 
       elif not Validacao_2 == int(CPF_Inserido[13]) or not Validacao_1 == int(CPF_Inserido[12]):
           print (' ')
           print ('[x] - Esse CPF não existe, por favor confira se digitou corretamente.')
       else:
         print (' ')
    ...
    if CPF_Unico == True and Nascimento_Valido == True:
        ...
        while Loop_Cad_2 == True:
            ...
            if Nome_Valido == True and PIN_Valido == False: #Tive que colocar que o PIN precisa ser falso aqui pra ele não perguntar a senha duas vezes, sei que fica estranho mas funciona.
                Senha_Escolhida = str(input('>Crie uma senha com no minimo 8 digitos, digitos e letras: '))
                if Senha_Escolhida.lower () == 'cancel':
                    ...
                    Loop_Cad_2 = False
                    break
                elif len(Senha_Escolhida) >= 8 and not Senha_Escolhida.isdigit() and not Senha_Escolhida.isalpha():
                    Senha_Valida = True
                else:
                    print ('[x] - Por favor, insira uma senha de pelo menos 8 digitos, contendo digitos e letras!')
                    print (' ')
            else:
                pass
            if Nome_Valido == False:
                ...
                if Nome_Escolhido.lower () == 'cancel':
                    ...
                    Loop_Cad_2 = False
                    break
                elif len(Nome_Escolhido) > 0 and Nome_Escolhido.isalpha():
                    Nome_Valido = True
                else:
