...
if Login_Valido == True:
    ...
    while Escolha_Valida01 == False:
        ...
        if entrada02 == '1':
            ...
            if str(Senha_Correto) == "('"+Senha_Validacao+"',)":
                ...
                while Emprestimo_Valido == False:
                    ...
                    if Emprestimo_Quantia[0].lower () == 'c':
                        ...
                        print ("(*IMPORTANTE!): Você receberá um emprestimo de "+str(Emprestimo_Quantia)+" com uma taxa de aumento de "+str(Info_Agencia[1])+"% por mês na devolução.")
            ...
            Emprestimo_Existente = ponteiro.fetchall ()
            if not Emprestimo_Existente:
                ...
                while True:
                    Confirmacao_Devolucao = input("Você deseja efetuar a devoluçaõ do emprestimo? [Y/N] ")
                    if Confirmacao_Devolucao[0].lower() == 'n':
                        print ('Cancelando operação... ')
                        print ('####--------------------------------------------------------------------####')
                        print ('\n')
                        Escolha_Valida01 = False
                        break
                    elif Confirmacao_Devolucao[0].lower() == 'y':
                        if float(str(Emprestimo_Existente[7])) <= float(str(Quantia_Atual[0])):
                            print ('SUCESSO!')
                        else:
                            print ('[x] - Devolução impossivel. Saldo insuficiente.')
                    elif Confirmacao_Devolucao[0].lower() == 'n':
                    else:
